(() => {
  ...
  function refresh() {
    ...
    multBy.innerText = "Mult By : " + ((parseInt(window.localStorage.getItem('mult')) + 1) * fbonus());
    multAnim.innerHTML = "x" + ((parseInt(window.localStorage.getItem('mult')) + 1) * fbonus());
    //section auto
    autoPrice.innerText = "Price : " + fautoPrice();
    autoRate.innerText = "Per sec : " + (parseInt(window.localStorage.getItem('auto-rate')) * fbonus());
    //section bonus
